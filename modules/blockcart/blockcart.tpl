	...
	{if $products}
		...
		{foreach from=$products item='product' name='myLoop'}
			...
			<dt id="cart_block_product_{$product.id_product}{if $product.id_product_attribute}_{$product.id_product_attribute}{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
			   <a href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)}"><img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'small')}" alt="{$product.name|escape:htmlall:'UTF-8'|truncate:20}" align="left" title="{$product.name|escape:htmlall:'UTF-8'|truncate:20}" /></a>               
				<span class="quantity-formated"><span class="quantity">{$product.cart_quantity}</span>x</span>
	         <div class="quantity_buttons">
					<a class="ajax_cart_quantity_up" rel="ajax_id_product_{$product.id_product}" href="{$base_dir_ssl}cart.php?add&amp;id_product={$product.id_product}&amp;ipa={$product.id_product_attribute}&amp;token={$static_token}" title="{l s='Add' mod='blockcart'}"><img src="{$img_dir}icon/quantity_up.gif" alt="{l s='Add' mod='blockcart'}" /></a>
					<a class="ajax_cart_quantity_down" rel="ajax_id_product_{$product.id_product}" href="{$base_dir_ssl}cart.php?add&amp;id_product={$product.id_product}&amp;ipa={$product.id_product_attribute}&amp;op=down&amp;token={$static_token}" title="{l s='Substract' mod='blockcart'}"><img src="{$img_dir}icon/quantity_down.gif" alt="{l s='Substract' mod='blockcart'}" /></a>
	         </div>				
				<a class="cart_block_product_name" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)}" title="{$product.name|escape:html:'UTF-8'}">{$product.name|truncate:8:'...'|escape:html:'UTF-8'}</a>
				<span class="remove_link">{if !isset($customizedDatas.$productId.$productAttributeId)}<a rel="nofollow" class="ajax_cart_block_remove_link" href="{$link->getPageLink('cart.php')}?delete&amp;id_product={$product.id_product}&amp;ipa={$product.id_product_attribute}&amp;token={$static_token}" title="{l s='remove this product from my cart' mod='blockcart'}">&nbsp;</a>{/if}</span><br /><br />
